...
public:
    ...
    float computeItemHeight(const TimelineItem& item, float card_w) const {
        ...
        auto p_title = b_title->Build();
        float badge_w = item.badge_text.empty() ? 0.0f : 55.0f;
        p_title->layout(std::max(60.0f, card_w - 24.0f - badge_w));
        h += p_title->getHeight() + 4.0f;

        // Timestamp
        if (!item.timestamp.empty()) {
            skia::textlayout::ParagraphStyle ts_style;
            auto b_time = skia::textlayout::ParagraphBuilder::make(ts_style, fc);
            skia::textlayout::TextStyle t_time;
            t_time.setFontSize(11.0f);
            b_time->pushStyle(t_time);
            b_time->addText(item.timestamp.c_str(), item.timestamp.length());
            auto p_time = b_time->Build();
            p_time->layout(card_w - 24.0f);
            h += p_time->getHeight() + 4.0f;
        }

        // Description
        if (!item.description.empty()) {
            skia::textlayout::ParagraphStyle ds_style;
            auto b_desc = skia::textlayout::ParagraphBuilder::make(ds_style, fc);
            skia::textlayout::TextStyle t_desc;
            t_desc.setFontSize(12.0f);
            b_desc->pushStyle(t_desc);
            b_desc->addText(item.description.c_str(), item.description.length());
            auto p_desc = b_desc->Build();
            p_desc->layout(card_w - 24.0f);
            h += p_desc->getHeight() + 6.0f;
        }

        // Expanded Details
        if (item.is_expanded && !item.details.empty()) {
            skia::textlayout::ParagraphStyle dt_style;
            auto b_det = skia::textlayout::ParagraphBuilder::make(dt_style, fc);
            skia::textlayout::TextStyle t_det;
            t_det.setFontSize(11.5f);
            b_det->pushStyle(t_det);
            b_det->addText(item.details.c_str(), item.details.length());
            auto p_det = b_det->Build();
            p_det->layout(card_w - 36.0f);
            h += p_det->getHeight() + 20.0f;
        }
    ...
    float computeTotalHeight() const {
        ...
        if (options.orientation == TimelineOrientation::Horizontal) {
            ...
            for (const auto& item : items) {
                max_card_h = std::max(max_card_h, computeItemHeight(item, options.card_width));
            }
        ...
        for (size_t i = 0; i < items.size(); ++i) {
            float item_h = computeItemHeight(items[i], card_w);
            total_h += item_h;
    ...
    void paintHorizontal(SkCanvas* sk_canvas, Point offset, sk_sp<skia::textlayout::FontCollection> fc) {
        ...
        for (size_t i = 0; i + 1 < n; ++i) {
            ...
            if (options.line_style == TimelineLineStyle::Dashed) {
                ...
                line_paint.setPathEffect(SkDashPathEffect::Make(intervals, 2, 0.0f));
            }
        ...
        for (size_t i = 0; i < n; ++i) {
            ...
            float card_h = computeItemHeight(item, card_w);

            layout_cache_[i].card_rect = Rect{card_x, card_y, card_w, card_h};
            paintCard(sk_canvas, card_x, card_y, card_w, card_h, item, (hovered_item_index == static_cast<int>(i)), fc);
        }
    }
    ...
    void paintVerticalStandard(SkCanvas* sk_canvas, Point offset, sk_sp<skia::textlayout::FontCollection> fc) {
        ...
        // Precompute centers
        for (size_t i = 0; i < n; ++i) {
            float card_h = computeItemHeight(items[i], card_w);
        ...
        for (size_t i = 0; i < n; ++i) {
            ...
            paintCard(sk_canvas, cache.card_rect.x, cache.card_rect.y, cache.card_rect.width, cache.card_rect.height,
                      item, (hovered_item_index == static_cast<int>(i)), fc);
        }
    }
    ...
    void paintNode(SkCanvas* sk_canvas, float cx, float cy, const TimelineItem& item, size_t index,
        ...
        if (item.status == TimelineItemStatus::Completed) {
            ...
            auto b_ic = skia::textlayout::ParagraphBuilder::make(p_style, fc);
            skia::textlayout::TextStyle t_ic;
            t_ic.setFontSize(options.node_size * 0.55f);
            b_ic->pushStyle(t_ic);
            b_ic->addText(item.icon.c_str(), item.icon.length());
            auto p_ic = b_ic->Build();
            p_ic->layout(options.node_size);
            p_ic->paint(sk_canvas, cx - options.node_size * 0.5f, cy - p_ic->getHeight() * 0.5f);
        } else if (options.is_stepper || item.node_shape == TimelineNodeShape::Number) {
            std::string num_str = std::to_string(index + 1);
            skia::textlayout::ParagraphStyle p_style;
            p_style.setTextAlign(skia::textlayout::TextAlign::kCenter);
            auto b_num = skia::textlayout::ParagraphBuilder::make(p_style, fc);
            skia::textlayout::TextStyle t_num;
            t_num.setFontSize(11.0f);
            t_num.setColor(item.status == TimelineItemStatus::Pending ? static_cast<SkColor>(options.pending_color) : 0xFF0F172A);
            b_num->pushStyle(t_num);
            b_num->addText(num_str.c_str(), num_str.length());
            auto p_num = b_num->Build();
            p_num->layout(options.node_size);
            p_num->paint(sk_canvas, cx - options.node_size * 0.5f, cy - p_num->getHeight() * 0.5f);
        }
    }

    void paintCard(SkCanvas* sk_canvas, float x, float y, float w, float h, const TimelineItem& item,
                   bool is_hov, sk_sp<skia::textlayout::FontCollection> fc) {
        SkRRect rrect;
        ...
        // Compute Badge first if present
        std::unique_ptr<skia::textlayout::Paragraph> p_bg;
        float badge_w = 0.0f;
        if (!item.badge_text.empty()) {
            auto b_bg = skia::textlayout::ParagraphBuilder::make(p_style, fc);
            skia::textlayout::TextStyle t_bg;
            t_bg.setFontSize(10.5f);
            t_bg.setColor(static_cast<SkColor>(item.badge_fg != 0 ? item.badge_fg : 0xFFFFFFFF));
            b_bg->pushStyle(t_bg);
            b_bg->addText(item.badge_text.c_str(), item.badge_text.length());
            p_bg = b_bg->Build();
            p_bg->layout(100.0f);
            badge_w = p_bg->getMaxIntrinsicWidth() + 14.0f;
        }
        ...
        b_title->addText(item.title.c_str(), item.title.length());
        auto p_title = b_title->Build();
        p_title->layout(std::max(40.0f, avail_w - badge_w));
        p_title->paint(sk_canvas, pad_x, cur_y);

        float title_h = p_title->getHeight();

        // Paint Badge on the right
        if (p_bg) {
            float bw = p_bg->getMaxIntrinsicWidth() + 10.0f;
            float bh = p_bg->getHeight() + 2.0f;
            ...
            p_bg->paint(sk_canvas, bx + 5.0f, by + 1.0f);
        }
        ...
        if (!item.timestamp.empty()) {
            ...
            skia::textlayout::TextStyle t_time;
            t_time.setFontSize(11.0f);
            t_time.setColor(static_cast<SkColor>(options.timestamp_color));
            b_time->pushStyle(t_time);
            b_time->addText(item.timestamp.c_str(), item.timestamp.length());
            auto p_time = b_time->Build();
            p_time->layout(avail_w);
            p_time->paint(sk_canvas, pad_x, cur_y);
            cur_y += p_time->getHeight() + 4.0f;
        }

        // 3. Description
        if (!item.description.empty()) {
            auto b_desc = skia::textlayout::ParagraphBuilder::make(p_style, fc);
            skia::textlayout::TextStyle t_desc;
            t_desc.setFontSize(12.0f);
            t_desc.setColor(static_cast<SkColor>(options.desc_color));
            b_desc->pushStyle(t_desc);
            b_desc->addText(item.description.c_str(), item.description.length());
            auto p_desc = b_desc->Build();
            p_desc->layout(avail_w);
            p_desc->paint(sk_canvas, pad_x, cur_y);
            cur_y += p_desc->getHeight() + 6.0f;
        }

        // 4. Expandable Details Box
        if (item.is_expanded && !item.details.empty()) {
            auto b_det = skia::textlayout::ParagraphBuilder::make(p_style, fc);
            skia::textlayout::TextStyle t_det;
            t_det.setFontSize(11.5f);
            t_det.setColor(0xFF94A3B8);
            b_det->pushStyle(t_det);
            b_det->addText(item.details.c_str(), item.details.length());
            auto p_det = b_det->Build();
            p_det->layout(avail_w - 16.0f);

            float det_h = p_det->getHeight() + 12.0f;
            SkRRect det_box;
            ...
            p_det->paint(sk_canvas, pad_x + 8.0f, cur_y + 6.0f);
        }
    }
