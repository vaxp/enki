...
public:
    ...
    // GPU-native glow: stacked semi-transparent expanded shapes.
    // Replaces SkImageFilters::Blur (which forces implicit CPU saveLayer).
    static void drawFakeGlowRRect(Canvas& c, const Rect& r, float radius,
                                  Color color, float spread, int rings = 3) {
        uint8_t base_a = (color >> 24) & 0xFF;
        for (int ring = rings; ring >= 1; --ring) {
            float e = spread * (static_cast<float>(ring) / rings);
            uint8_t a = static_cast<uint8_t>(base_a * (1.0f - static_cast<float>(ring - 1) / rings) * 0.55f);
            Color rc = (color & 0x00FFFFFFu) | (static_cast<uint32_t>(a) << 24);
            Paint p; p.setColor(rc); p.setAntiAlias(true);
            Rect er{r.x - e, r.y - e, r.width + e * 2.0f, r.height + e * 2.0f};
            c.drawRRect(er, BorderRadius::circular(radius + e), p);
        }
    }

    static void drawFakeGlowCircle(Canvas& c, Point center, float base_radius,
                                   Color color, float spread, int rings = 3) {
        uint8_t base_a = (color >> 24) & 0xFF;
        for (int ring = rings; ring >= 1; --ring) {
            float e = spread * (static_cast<float>(ring) / rings);
            ...
            Color rc = (color & 0x00FFFFFFu) | (static_cast<uint32_t>(a) << 24);
            Paint p; p.setColor(rc); p.setAntiAlias(true);
            c.drawCircle(center, base_radius + e, p);
        }
    }
    ...
    void paint(PaintContext& ctx) override {
        ...
        // GPU-native: stacked rings instead of SkImageFilters::Blur
        if (options.style == NavigationBarStyle::FloatingPill || options.enable_glassmorphism) {
            ...
            drawFakeGlowRRect(ctx.canvas, shadow_rect, options.corner_radius,
                              options.shadow_color, options.shadow_blur, 4);

            // Subtle backdrop tint (was 0x1A38BDF8 blurred — single low-alpha rect is equivalent)
            Paint gp;
        ...
        if (options.indicator_style != NavIndicatorStyle::None) {
            ...
            if (ind_target.width > 0.0f) {
                ...
                if (options.indicator_style == NavIndicatorStyle::Pill) {
                    // GPU-native glow rings instead of blur filter
                    drawFakeGlowRRect(ctx.canvas, ind_world, options.indicator_radius,
                                      options.glow_color, 7.0f, 3);
                    ...
                    // GPU-native glow for underline
                    drawFakeGlowRRect(ctx.canvas, ind_world,
                                      options.indicator_thickness * 0.5f,
                                      options.active_color, 5.0f, 2);
                    ...
                    // GPU-native glow rings for dot
                    drawFakeGlowCircle(ctx.canvas, dot_c, 4.0f, options.glow_color, 5.0f, 3);
        ...
        for (int i = 0; i < n; ++i) {
            ...
            if (item.dot_badge) {
                ...
                // GPU-native glow rings for badge dot
                drawFakeGlowCircle(ctx.canvas, dot_p, 4.0f, options.badge_color, 4.0f, 2);
                ...
                // GPU-native glow rings for badge label
                drawFakeGlowRRect(ctx.canvas, b_rect, bh * 0.5f, 0x66EF4444, 4.0f, 2);
        ...
        if (options.show_tooltips && hovered_index >= 0 && hovered_index < n) {
            ...
            if (!tip.empty()) {
                ...
                // GPU-native shadow rings for tooltip
                drawFakeGlowRRect(ctx.canvas, tip_r, 4.0f, 0x88000000, 5.0f, 2);
